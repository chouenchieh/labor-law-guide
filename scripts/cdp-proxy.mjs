...
async function discoverChromePort() {
  ...
  for (const port of commonPorts) {
    const ok = await checkPort(port);
    if (ok) {
      // 从 /json/version 获取完整 wsPath（Chrome 可能要求带 UUID 的路径）
      let resolvedWsPath = null;
      try {
        const resp = await fetch(`http://127.0.0.1:${port}/json/version`);
        const json = await resp.json();
        if (json.webSocketDebuggerUrl) {
          const u = new URL(json.webSocketDebuggerUrl);
          resolvedWsPath = u.pathname;
        }
      } catch {}
      console.log(`[CDP Proxy] 扫描发现 Chrome 调试端口: ${port}${resolvedWsPath ? " (wsPath=" + resolvedWsPath + ")" : ""}`);
      return { port, wsPath: resolvedWsPath };
    }
  }
